    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-ec-muted">Select your checked baggage options</p>
        </div>
        {totalBaggagePrice > 0 && (
          <div className="text-right">
            <div className="text-sm text-ec-muted">Baggage</div>
            <div className="text-lg font-semibold text-ec-text">{currency} {totalBaggagePrice.toFixed(2)}</div>
          </div>
        )}
      </div>
      ...
      <div className="p-4 bg-[rgba(28,140,130,0.1)] rounded-ec-md border border-[rgba(28,140,130,0.2)]">
        <div className="text-sm font-semibold text-ec-text mb-1">Included:</div>
        <div className="text-sm text-ec-muted">1 × Carry-on bag (7kg, fits overhead)</div>
      </div>
        ...
        {baggageOptions.map((option) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <div className="text-lg font-semibold text-ec-text mb-1">{option.label}</div>
              ...
              <div className="text-xl font-bold text-ec-text">{currency} {option.price.toFixed(2)}</div>
            </div>
          </button>
        ))}
      </div>
          ...
          <div className="space-y-3">
            {baggage.checkedBags.map((bag, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-[rgba(15,17,20,0.5)] rounded-lg border border-[rgba(28,140,130,0.2)]">
                <div className="text-base font-medium text-ec-text">
                  {bag.type === '20kg' && '20kg Checked Bag'}
                  {bag.type === '30kg' && '30kg Checked Bag'}
                  {bag.type === 'extra' && 'Extra Bag'}
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-base text-ec-text font-semibold">{currency} {bag.price.toFixed(2)}</span>
                  <button
                    type="button"
