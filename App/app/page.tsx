...
export default function Home() {
  ...
  const handleSearch = async () => {
    ...
    setResults([]);
    try {
      const url = `/api/flights/search?from=${from}&to=${to}&departDate=${departDate}&adults=${adults}&cabinClass=${cabinClass}&currency=${currency}${returnDate ? `&returnDate=${returnDate}&tripType=roundtrip` : ''}`;
  ...
  return (
    <>
      ...
      {!loading && !error && results.length === 0 && (
        <EcoviraCard variant="glass" className="p-12 md:p-16 lg:p-20 text-center mt-20">
          <div className="text-6xl mb-8 text-ec-muted font-light">Ready</div>
          <h3 className="text-3xl md:text-4xl font-serif font-semibold text-ec-text mb-6">
            Ready to search
          </h3>
          <p className="text-xl text-ec-muted max-w-lg mx-auto">
            Enter your travel details above to find premium flights.
          </p>
        </EcoviraCard>
      )}
    </>
