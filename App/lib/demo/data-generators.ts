  ...
  if (returnDate) {
    for (let i = 0; i < 8; i++) {
      ...
      results.push({
        ...
        currency: 'AUD',
        cabinClass,
        fareType: i % 2 === 0 ? 'Economy' : 'Economy',
        baggage: '1 checked bag included',
        refundable: i % 4 === 0,
        valueScore: 80 + (i % 20),
        raw: { demo: true, index: i, return: true },
      });
    }
  }
...
export function generateDemoCars(params: {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    results.push({
      ...
      id: `demo-car-${i}`,
      provider,
      carClass: carType.class,
      carName: `${provider} ${carType.name}`,
      seats: carType.seats,
...
export function generateDemoTransfers(params: {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    results.push({
      ...
      pickupDate,
      pickupTime,
      price: (isShared ? vehicle.price * 0.6 : vehicle.price).toString(),
      currency: 'AUD',
      shared: isShared,
      duration: `${30 + (i * 5)} minutes`,
      raw: { demo: true, index: i },
    });
  }
